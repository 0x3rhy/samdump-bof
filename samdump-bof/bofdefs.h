...
#define STATUS_NOT_ALL_ASSIGNED 0x00000106
	...
	DECLSPEC_IMPORT LSTATUS APIENTRY ADVAPI32$RegSaveKeyA( HKEY hKey,  LPCSTR lpFile, CONST LPSECURITY_ATTRIBUTES lpSecurityAttributes);
	...
	DECLSPEC_IMPORT BOOL WINAPI KERNEL32$ReadFileEx(HANDLE hFile, LPVOID lpBuffer, DWORD nNumberOfBytesToRead, LPOVERLAPPED lpOverlapped, LPOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine);
	...
	DECLSPEC_IMPORT BOOL WINAPI KERNEL32$GetFileAttributesA(LPCSTR lpFileName);
	...
	DECLSPEC_IMPORT LPSTR WINAPI SHLWAPI$PathCombineA(LPSTR  pszDest, LPCSTR pszDir, LPCSTR pszFile);
	...
	DECLSPEC_IMPORT LPWSTR WINAPI	Kernel32$lstrcatW(LPWSTR lpString1, LPWSTR lpString2);
	...
	/* Random */
	DECLSPEC_IMPORT void WINAPI MSVCRT$srand(unsigned int initial);
	...
	DECLSPEC_IMPORT NTSTATUS WINAPI NTDLL$NtDelayExecution(BOOLEAN  Alertable, PLARGE_INTEGER  Interval);
	DECLSPEC_IMPORT NTSTATUS WINAPI NTDLL$ZwSetTimerResolution(IN ULONG RequestedResolution, IN BOOLEAN Set, OUT PULONG ActualResolution);
...
#define ReadFileEx			KERNEL32$ReadFileEx
...
#define PathCombineA SHLWAPI$PathCombineA
...
#define NtDelayExecution NTDLL$NtDelayExecution
#define ZwSetTimerResolution NTDLL$ZwSetTimerResolution
